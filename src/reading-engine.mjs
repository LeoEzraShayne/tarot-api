...
function buildInterpretation(s,locale='en'){
...
export function deterministicInterpretation(id,locale='en'){
  ...
  const result=buildInterpretation(s,locale);
  s.state='interpreted';
...
export function localizeStoredInterpretation(saved,locale='en'){
  ...
  const snapshot={id:'stored',question:saved.question,spreadId:saved.spread.id,state:'revealed',seed:'stored',order:DATASET.cardCount?Array.from({length:DATASET.cardCount},(_,index)=>String(index)):[],orientations:Object.fromEntries(saved.draw.map(card=>[String(card.cardId),card.orientation])),selections,createdAt:new Date(0).toISOString(),engineVersion:saved.engineVersion||'1.0.0'};
  return buildInterpretation(snapshot,locale);
}
